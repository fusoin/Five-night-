...
var elapsed := 0.0
var power := 100.0
# Insanity is a survivability resource: the player starts at 100% and dies at 0%.
var insanity := 100.0
var camera_up := false
var camera_index := 0
var left_door := false
...
func _show_menu() -> void:
	...
	_create_label(self, "Press 1-6 to choose a night", Vector2(0, 600), Vector2(1280, 32), 22)
...
func _add_anim(name: String, side: String, ai: int) -> void:
	anims.append({"name": name, "side": side, "ai": ai, "phase": 0, "timer": randf_range(2.0, 7.5), "laughs": 0})

func _build_game_ui() -> void:
	for child in get_children(): child.queue_free()
	ui.clear()
	var bg := ColorRect.new(); bg.color = Color("11131a"); bg.set_anchors_preset(Control.PRESET_FULL_RECT); add_child(bg)
	var office := ColorRect.new(); office.color = Color("1d1d20"); office.position = Vector2(160, 100); office.size = Vector2(960, 520); add_child(office); office_rect = office
	var desk := ColorRect.new(); desk.color = Color("302a24"); desk.position = Vector2(480, 480); desk.size = Vector2(320, 130); add_child(desk)
	left_door_rect = ColorRect.new(); left_door_rect.position = Vector2(175, 120); left_door_rect.size = Vector2(110, 480); add_child(left_door_rect)
	right_door_rect = ColorRect.new(); right_door_rect.position = Vector2(995, 120); right_door_rect.size = Vector2(110, 480); add_child(right_door_rect)
	...
	ui["insanity"] = _create_label(self, "Insanity: 100%", Vector2(40, 20), Vector2(250, 35), 22)
	...
	ui["tip"] = _create_label(self, "Tip: Insanity reaches 0% = death.", Vector2(860, 60), Vector2(340, 40), 17)
	ui["phone"] = _create_label(self, "", Vector2(30, 470), Vector2(350, 150), 16)
	ui["status"] = _create_label(self, "Night %d started. Keep your insanity above 0%%." % night, Vector2(160, 640), Vector2(960, 40), 20)
	_update_ui()
...
func _process(delta: float) -> void:
	if screen == "menu":
		if Input.is_key_pressed(KEY_N):
			...
			if "hold_progress" in ui: ui["hold_progress"].text = "Skipping notice: %d%%" % int(min(100.0, title_skip_hold / 1.2 * 100.0))
			if title_skip_hold >= 1.2: _start_night(1)
		else:
			title_skip_hold = 0.0
			if "hold_progress" in ui: ui["hold_progress"].text = ""
		return
	...
	# Insanity slowly falls throughout the night; looking through cameras accelerates it.
	insanity = max(0.0, insanity - delta * (1.2 if camera_up else 0.7))
	if insanity <= 0.0:
		_lose("Your insanity reached 0%. The darkness took over.")
		return
	if power <= 0.0:
		_lose("The power died. Something is breathing in the dark.")
		return
	if not phone_shown and elapsed >= 3.0:
		phone_shown = true; ui["phone"].text = "PHONE CALL\n" + PHONE_CALLS[night]
...
func _update_anim_ai(delta: float) -> void:
	for anim in anims:
		...
		anim["phase"] += 1
		if anim["name"] == "Freddy":
			anim["laughs"] += 1; ui["status"].text = "Freddy laughs %d/5. Keep the flashlight ready." % anim["laughs"]
		if anim["phase"] >= 4:
			if anim["name"] == "Foxy" or anim["name"] == "Afton":
				if left_door or right_door: _lose(anim["name"] + " hit the door. Flash the hallway sooner next time.")
				else: _lose(anim["name"] + " is in the office.")
				return
			if anim["side"] == "left" and not left_door: _lose(anim["name"] + " reached the left side open door."); return
			if anim["side"] == "right" and not right_door: _lose(anim["name"] + " reached the right side open door."); return
			anim["phase"] = 2

func _update_ui() -> void:
	if "clock" in ui:
		ui["clock"].text = ["12 AM", "1 AM", "2 AM", "3 AM", "4 AM", "5 AM", "6 AM"][int(min(6, elapsed / 15.0))]
	if "power" in ui: ui["power"].text = "Power: %d%%" % int(power)
	if "insanity" in ui: ui["insanity"].text = "Insanity: %d%%" % int(insanity)
	if "door_status" in ui: ui["door_status"].text = "Left: %s    Right: %s" % ["CLOSED" if left_door else "OPEN", "CLOSED" if right_door else "OPEN"]
	if left_door_rect: left_door_rect.color = Color("9fd1ff") if left_door else Color("28282d")
	if right_door_rect: right_door_rect.color = Color("9fd1ff") if right_door else Color("28282d")
	if "camera" in ui:
		if camera_up:
			var movement := "No movement detected."
			for anim in anims:
				if anim["phase"] > 0: movement = "%s: phase %d" % [anim["name"], anim["phase"]]; break
			ui["camera"].text = "Camera: %s\n%s\n\n[Q/E] Change camera  [SPACE] Flash" % [ROOMS[camera_index], movement]
		else: ui["camera"].text = "Camera: Off"

func _unhandled_input(event: InputEvent) -> void:
	if event is not InputEventKey or not event.pressed or event.echo: return
	if screen == "menu":
		if event.keycode >= KEY_1 and event.keycode <= KEY_6: _start_night(int(event.keycode - KEY_0))
		return
	if screen != "game" or game_over or won:
		if event.keycode == KEY_R: _show_menu()
		return
	match event.keycode:
		KEY_A: left_door = not left_door
		KEY_D: right_door = not right_door
		...
		KEY_1, KEY_2, KEY_3, KEY_4: _eat_snack(event.keycode == KEY_2 or event.keycode == KEY_4)
	_update_ui()

func _eat_snack(opened: bool) -> void:
	if opened: insanity = min(100.0, insanity + 10.0); ui["status"].text = "You ate an open snack. Insanity +10%."
	else: insanity = min(100.0, insanity + 10.0); ui["status"].text = "You ate a sealed snack. Insanity restored +10%."

func _flash() -> void:
	for anim in anims:
		if anim["name"] in ["Foxy", "Freddy", "Chica", "Afton"] and anim["phase"] > 0:
			anim["phase"] = 0; anim["timer"] = 8.0
...
func _won_screen() -> void:
	...
	if night == 6:
		_create_label(self, "You beat all six nights!\nYour check: $2,228", Vector2(0, 340), Vector2(1280, 100), 26)
	else:
		_create_label(self, "Night %d complete.\nNo check yet — beat Night 6 to get paid." % night, Vector2(0, 340), Vector2(1280, 100), 26)
